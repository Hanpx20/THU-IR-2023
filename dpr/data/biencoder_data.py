...
from dpr.utils.data_utils import read_data_from_json_files, read_data_from_jsonl_files, Dataset
...
class JsonlistQADataset(Dataset):
    ...
    def _load_all_data(self):
        ...
        # filter those without positive ctx
        self.data = [r for r in data if len(r["SS"]) > 0]
        logger.info("Total cleaned data size: %d", len(self.data))
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        positive_ctxs = [{"title": json_sample["title"], "text": json_sample["SS"]}]
        if self.exclude_gold:
            ctxs = [ctx for ctx in positive_ctxs if "score" in ctx]
            if ctxs:
                positive_ctxs = ctxs
